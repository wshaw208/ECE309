...
public class NekoTheCat implements MouseListener, Runnable {
	...
	public void run() {
		...
		while(true)
		{
			...
			if(!((catxPosition > 0) && (catxPosition < gamePanel.getSize().width)))
			{
				xBump = ~xBump;
				if(xBump > 0)
				{
			...
			// 6. If necessary, redirect Neko toward the ball.
			if(ballHasBeenPlaced)
			{
				if(catyPosition < ballyPosition)
				{
					catyPosition += catHeight;
				}
			...
			if((Math.abs(catyPosition - ballyPosition) < catHeight) && (Math.abs(catxPosition - ballxPosition) < xBump))
			{
				...
				g.drawString("At last, I have my ball!",0,0);
				soundFile.stop();
				return;
			}
		}
